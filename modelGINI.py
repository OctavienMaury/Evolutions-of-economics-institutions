...
class Institution(Agent):
    def __init__(self, unique_id, model):
        ...
        self.power_concentration = 1.5
        self.gini_index = 0.7

    def calculate_gini_index(self):
        influences = [agent.influence for agent in self.model.schedule.agents if not isinstance(agent, Institution)]

        #print(f"Influences des agents : {influences}")

        if len(influences) == 0:
            return 0  # Eviter la division par zéro si pas d'agents

        influences_sorted = sorted(influences)
        n = len(influences_sorted)

        cumulative_influences = np.cumsum(influences_sorted)
        total_sum = np.sum(influences_sorted)
        if total_sum == 0:
            return 0

        self.gini_index = (2 * np.sum((np.arange(1, n+1) * influences_sorted)) - (n + 1) * np.sum(influences_sorted)) / (n * total_sum)

        #print(f"Indice de Gini actuel : {self.gini_index}")

        return self.gini_index

    def apply_power_concentration(self):
        for agent in self.model.schedule.agents:
            if not isinstance(agent, Institution):
                # Plus l'agent est influent, plus il reçoit un bonus d'influence
                influence_boost = agent.influence * self.power_concentration * 0.01
                agent.influence = min(agent.influence + influence_boost, 500)  # Limiter l'influence à un maximum de 100
    ...
    def step(self):
        ...
        self.apply_power_concentration()

        gini_index = self.calculate_gini_index()
        #print(f"Gini index for influence: {gini_index}")
...
class Innovator(Agent):
    def __init__(self, unique_id, model):
        ...
        self.revolution_threshold = random.randint(50, 200)
        self.satisfaction_threshold = random.randint(50, 200)
        self.ideas = set()
    ...
    def step(self):
        ...
        for neighbor in neighbors:
            if not isinstance(neighbor, Institution):
                ...
                initial_idea_count = len(neighbor.ideas)
                neighbor.ideas.update(self.ideas)

                if len(neighbor.ideas) > initial_idea_count:
                    self.influence = min(self.influence + 0.01, 50)  # Augmentation rapide de l'influence si propagation d'idées

                neighbor.satisfaction_threshold = min(300, max(1, neighbor.satisfaction_threshold + self.knowledge * 0.03))
                neighbor.revolution_threshold = min(300, max(1, neighbor.revolution_threshold - self.knowledge * 0.03))
...
class Noble(Agent):
    ...
    def step(self):
        ...
        self.wealth += self.model.institution.power_concentration * 0.01
      
        if self.model.institution.property_rights > 0: 
            self.wealth += self.model.institution.property_rights * 0.1

        self.revolution_threshold = min(300, max(1, self.revolution_threshold + self.wealth * 0.03))
        self.satisfaction_threshold = min(300, max(1, self.satisfaction_threshold - self.wealth * 0.03))
        ...
        for neighbor in neighbors:
            if not isinstance(neighbor, Institution):
                neighbor.satisfaction_threshold = min(300, max(1, neighbor.satisfaction_threshold - self.influence * 0.016))
                neighbor.revolution_threshold = min(300, max(1, neighbor.revolution_threshold + self.influence * 0.03))
...
class Bourgeois(Agent):
    ...
    def step(self):
        ...
        if self.model.institution.property_rights > sum(1 for agent in self.model.schedule.agents if isinstance(agent, Noble)):
            self.wealth += self.model.institution.property_rights * 0.01  
        ...
        for neighbor in neighbors:
            if not isinstance(neighbor, Institution):
                neighbor.satisfaction_threshold = min(300, max(1, neighbor.satisfaction_threshold - self.knowledge * 0.03 - self.model.institution.property_rights * 0.03 - self.wealth * 0.03))    
                neighbor.revolution_threshold = min(300, max(1, neighbor.revolution_threshold - self.knowledge * 0.03 + self.model.institution.property_rights * 0.03 + self.wealth * 0.03))
...
class People(Agent):
    ...
    def step(self):
        ...
        if self.model.institution.property_rights > sum(1 for agent in self.model.schedule.agents if isinstance(agent, Noble)):
            self.wealth += self.model.institution.property_rights * 0.001
        ...
        for neighbor in neighbors:
            if not isinstance(neighbor, Institution):
                neighbor.satisfaction_threshold = min(300, max(1, neighbor.satisfaction_threshold - self.knowledge * 0.03 - self.model.institution.property_rights * 0.03 - self.wealth * 0.3))
                neighbor.revolution_threshold = min(300, max(1, neighbor.revolution_threshold - self.knowledge * 0.03 + self.model.institution.property_rights * 0.03 + self.wealth * 0.3))
...
class InstitutionModel(Model):
    def __init__(self, N, G):
        ...
        self.datacollector = DataCollector(
            model_reporters={"PropertyRights": lambda m: m.institution.property_rights, "Revolutionaries": lambda m: m.institution.revolutionaries, "Satisfaction": lambda m: m.institution.satisfaction, "Gini": lambda m: m.institution.gini_index},
...
def save_at_step(step):
    ...
    # Graphique des révolutionnaires
    ...
    # Graphique du seuil de révolution
    ...
    # Graphique des seuils de satisfaction
    agent_data = model.datacollector.get_agent_vars_dataframe()
  ...
  #Gini
    ...
    plt.plot(gini_data.index, gini_data.values, label="Indice de Gini", color='blue', alpha=0.7)
    plt.title(f"Évolution de l'Indice de Gini jusqu'à l'étape {step}")
    plt.xlabel("Étape")
    plt.ylabel("Indice de Gini")
    plt.legend()
    plt.grid(True)
    plt.savefig(f"{output_folder}/gini_evolution_step_{step}.png")
    plt.close()
...
for i in range(1, 5000):
    model.step()
    if i in [10, 50, 99, 100, 150, 200, 250, 500, 600, 700, 800, 900, 999, 1000, 1200, 1500, 1700, 1999, 2500, 2999, 3500, 3999, 4500, 4999, 5500, 5999, 6500, 6999, 7500, 7999, 8500, 8999, 9500, 9999]:
        save_at_step(i)
